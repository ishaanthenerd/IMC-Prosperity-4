...
class AshCoatedOsmium(Product):
    def __init__(self, symbol: str, limit: int, state: TradingState, fixed_mean: float, fixed_std: float):
        ...
        self.rolling_z = RollingZ(z_th = 1.5, window = 20, fixed_mean = fixed_mean)
...
class IntarianPepperRoot(Product):
    ...
    def strategy(self):
        best = self.best_ask()
        if not math.isnan(best) and self.best_ask() < 12015 and self.active_position() < self.limit:
            self.buy(best, -self.order_depth.sell_orders[best], print=True)
