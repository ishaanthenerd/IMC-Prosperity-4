...
class AshCoatedOsmium(Product):
    ...
    def fair_val(self):
        return int(self.mid_price_using_best()) if not math.isnan(self.mid_price_using_best()) else math.nan
    
    def taking_threshold_for_position(self, pos: int) -> int:
        ...
        util = abs(pos) / self.limit if self.limit else 0.0
        th = 0
    ...
    def strategy(self):
        # refactored from R1_davis_fallbacks.py

        if math.isnan(self.best_bid()) and math.isnan(self.best_ask()):
            return
        ...
        taking_th = self.taking_threshold_for_position(pos)
        cheap_enough = (not math.isnan(self.best_ask())) and (self.best_ask() < self.fair_val() - taking_th)
        rich_enough = (not math.isnan(self.best_bid())) and (self.best_bid() > self.fair_val() + taking_th)
        ...
        if cheap_enough and pos < 0:
            for price in sorted(self.order_depth.sell_orders.keys()):
                if price >= self.fair_val() - taking_th:
                    continue
                vol = -self.order_depth.sell_orders[price]
                if vol <= 0:
                    continue
                room = min(self.limit - sim, -sim)
                if room <= 0:
                    break
                q = min(vol, room)
                if q <= 0:
        ...
        if rich_enough and pos > 0:
            for price in sorted(self.order_depth.buy_orders.keys(), reverse=True):
                if price <= self.fair_val() + taking_th:
                    continue
                vol = self.order_depth.buy_orders[price]
                if vol <= 0:
                    continue
                room = min(sim + self.limit, sim)
                if room <= 0:
                    break
                q = min(vol, room)
                if q <= 0:
        ...
        rem_buy_batch = max(0, self.limit - pos - buy_vol_takes)
        rem_sell_batch = max(0, pos + self.limit - sell_vol_takes)

        lo, hi = self.fair_val() - 3, self.fair_val() + 3
        bids_outside = [p for p in self.order_depth.buy_orders if p < lo or p > hi]
        asks_outside = [p for p in self.order_depth.sell_orders if p < lo or p > hi]

        if bids_outside:
            mm_bid = max(bids_outside) + 1
        else:
            mm_bid = self.fair_val()

        if asks_outside:
            mm_ask = min(asks_outside) - 1
        else:
            mm_ask = self.fair_val()

        if mm_bid >= mm_ask:
            mm_ask = mm_bid + 1

        mm_buy_sz = min(max(0, self.limit - pos_after_takes), rem_buy_batch)
        mm_sell_sz = min(max(0, pos_after_takes + self.limit), rem_sell_batch)

        mm_orders: list[Order] = []
        if mm_buy_sz > 0 and not math.isnan(mm_bid):
            mm_orders.append(Order("ASH_COATED_OSMIUM", mm_bid, mm_buy_sz))
        if mm_sell_sz > 0 and not math.isnan(mm_ask):
...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        if not Trader.turned_on:
            ...
            product_instances["ASH_COATED_OSMIUM"] = AshCoatedOsmium("ASH_COATED_OSMIUM", 80, state, 10000, 4.5)
