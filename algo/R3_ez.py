...
class Hydrogel(Product):
    def __init__(self, product, limit, state):
        super().__init__(product, limit, state)
        self.rolling_z = RollingZ(z_th=2.0, window=20)
    
    def fair_val(self):
        return self.rolling_z.mean()
    
    def strategy(self):
        mid_price = self.mid_price_using_best()
        if not math.isnan(mid_price):
            self.rolling_z.add(mid_price)
        
        # Ensure the rolling window is full before making any trading decisions
        if len(self.rolling_z.premiums) < self.rolling_z.window:
            # logger.print(f"Hydrogel: Warming up window ({len(self.rolling_z.premiums)}/{self.rolling_z.window})")
            return

        signal = self.rolling_z.signal()
        fair_val = self.fair_val()
        std = self.rolling_z.std()
        
        if signal == 1:
            # Underpriced relative to rolling window -> Buy
            # buy or sell as long as the price is still within a certain threshold (e.g. z < -1.5)
            for price, qty in sorted(self.order_depth.sell_orders.items()):
                z = (price - fair_val) / std if std > 0 else 0
                if z < -1.5: # signal threshold is 2.0, exit/limit is 1.5
                    buy_quantity = min(self.limit_buy_orders(), -qty)
                    if buy_quantity > 0:
                        self.buy(price, buy_quantity)
                else:
                    break
        elif signal == -1:
            # Overpriced relative to rolling window -> Sell
            for price, qty in sorted(self.order_depth.buy_orders.items(), reverse=True):
                z = (price - fair_val) / std if std > 0 else 0
                if z > 1.5:
                    sell_quantity = min(self.limit_sell_orders(), qty)
                    if sell_quantity > 0:
                        self.sell(price, sell_quantity)
                else:
                    break
        else:
            # no signal and position is > 10, try to make 0 ev trades to reset position to 0
            if abs(self.position) > 10:
                if self.position > 10:
                    # try to sell to reset position to 0 at fair_val or better
                    # first take any bids >= fair_val
                    for price, qty in sorted(self.order_depth.buy_orders.items(), reverse=True):
                        if price >= fair_val:
                            ...
                            break
                    # then make an order at fair_val if we still have position
                    if self.position > 0:
                        self.sell(math.ceil(fair_val), self.position)
                elif self.position < -10:
                    # try to buy to reset position to 0 at fair_val or better
                    for price, qty in sorted(self.order_depth.sell_orders.items()):
                        if price <= fair_val:
                            buy_qty = min(-self.position, -qty)
                            self.buy(price, buy_qty)
                        else:
                            break
                    if self.position < 0:
                        self.buy(math.floor(fair_val), -self.position)
