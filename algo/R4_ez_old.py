...
class IVScalperOption(Product):
    def __init__(self, symbol: str, limit: int, state: TradingState, 
        ...
        self.b = 0.007299071308858503
        self.c = 0.27282114518021455
...
FOLLOW_TRADERS = {
    "VELVETFRUIT_EXTRACT": ["Mark 01", "Mark 14"],
    "VEV_4000": ["Mark 14"],
...
class VelvetFruitExtract(Product):
    ...
    def strategy(self):
        ...
        prev_timestamp = self.timestamp - 100
        slope = self.get_rolling_slope(SLOPE_WINDOW)
        
        traders_to_follow = FOLLOW_TRADERS.get(self.product, [])
        bought_trades = [t for t in market_trades if t.buyer in traders_to_follow and t.timestamp == prev_timestamp and abs(t.price - self.best_bid()) <= 2]
        sold_trades = [t for t in market_trades if t.seller in traders_to_follow and t.timestamp == prev_timestamp and abs(t.price - self.best_bid()) <= 2]

        # Calculate max volume to copy (matching their trade size)
        max_copy_vol = 0
        if bought_trades:
            max_copy_vol = sum(t.quantity for t in bought_trades)
        elif sold_trades:
            max_copy_vol = sum(t.quantity for t in sold_trades)

        if bought_trades and slope > 0:
            self.target_position = min(self.limit, self.active_position() + max_copy_vol)
            self.entry_price = max(t.price for t in bought_trades)
        elif sold_trades and slope < 0:
            self.target_position = max(-self.limit, self.active_position() - max_copy_vol)
            self.entry_price = min(t.price for t in sold_trades)
        
        if self.target_position > self.active_position():
            # Buy close to the price they traded at (entry_price + buffer)
            buy_price = min(self.best_ask(), self.entry_price + 2)
            self.buy(buy_price, min(self.limit_buy_orders(), self.target_position - self.active_position()))
        elif self.target_position < self.active_position():
            # Sell close to the price they traded at (entry_price - buffer)
            sell_price = max(self.best_bid(), self.entry_price - 2)
            self.sell(sell_price, min(self.limit_sell_orders(), self.active_position() - self.target_position))

        # Use 0 EV clear() to work back towards 0 if we aren't at 0
        # Only clear if we are not at 0 AND our position is large (absolute value > 20)
        if self.active_position() != 0:
            if abs(self.active_position()) > 20:
...
class VEV(IVScalperOption):
    ...
    def strategy(self):
        if self.product in FOLLOW_TRADERS:
            market_trades = self.state.market_trades.get(self.product, [])
            prev_timestamp = self.timestamp - 100
            slope = self.get_rolling_slope(SLOPE_WINDOW)
            ...
            if self.target_position > self.active_position():
                self.cancel_orders()
                # Buy close to the price they traded at
                buy_price = min(self.best_ask(), self.entry_price + 1)
                self.buy(buy_price, min(self.limit_buy_orders(), self.target_position - self.active_position()))
            elif self.target_position < self.active_position():
                self.cancel_orders()
                # Sell close to the price they traded at
                sell_price = max(self.best_bid(), self.entry_price - 1)
                self.sell(sell_price, min(self.limit_sell_orders(), self.active_position() - self.target_position))
...
class Hydrogel(Product):
    ...
    def strategy(self):
        # The prompt says "copy Mark 14 as long as the bid price is close enough to what he bought/sold for"
        market_trades = self.state.market_trades.get(self.product, [])
        prev_timestamp = self.timestamp - 100
        slope = self.get_rolling_slope(SLOPE_WINDOW)
        ...
        # Use 0 EV clear() to work back towards 0 if we aren't at 0
        # Only clear if we are not at 0 and our position is large
        if self.active_position() != 0:
            if abs(self.active_position()) > 20:
                self.clear(self.fair_val())
        else:
            # Fallback to the original strategy if we are at our target?
            # They probably wanted it to override but keep the take_clear_make if flat or we can just leave it as purely this strategy.
            # I'll preserve the old one inside the else branch for safety or just leave it.
            # actually usually when people say "do X" they mean replace it.
            pass
