...
class IVScalperOption(Product):
    def __init__(self, symbol: str, limit: int, state: TradingState, 
        ...
        self.deltas = RollingZ(1, 1000) # the 1 is unused
    ...
    def strategy(self):
        fv = self.fair_val()
        if self.is_call:
            delta = BlackScholes.delta_call(self.underlying.mid_price_using_best(),
                                            self.strike,
                                            self.tte,
                                            self.implied_volatility())
        else:
            delta = BlackScholes.delta_put(self.underlying.mid_price_using_best(),
                                            self.strike,
                                            self.tte,
                                            self.implied_volatility())
        self.theos.add(fv)
        ...
        # delta hedging ... actually costs us a ton of money so hell naw
        '''
        new_position = self.active_position()
        position_change = new_position - old_position
        if self.is_call:
            if position_change > 0:
                # long calls → positive delta → hedge by selling underlying
                self.underlying.full_sell(int(ceil(position_change * delta)))
            elif position_change < 0:
                # short calls → negative delta → hedge by buying underlying
                self.underlying.full_buy(int(floor(-position_change * delta)))
        else:
            if position_change > 0:
                # long puts → negative delta → hedge by buying underlying
                self.underlying.full_buy(int(ceil(position_change * -delta)))
            elif position_change < 0:
                # short puts → positive delta → hedge by selling underlying
                self.underlying.full_sell(int(floor(-position_change * -delta)))
        '''
...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        if not Trader.turned_on:
            ...
            # we're not trading 5200 and 5300; build out the z_th and mm_th
            strike_to_z = {
                4000: 1,
                4500: 1,
                5000: 1.5,
