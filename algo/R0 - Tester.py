...
'''
NOTE: This file used old code from Mathematican't in IMC Prosperity 3. Use the updated R0!
'''
...
class Product():
    ...
    def mid_price(self):
        return self.mid_price_using_best() # default is use best
...
# NOTE: this class will not function properly because new instances of classes are being made every time!
class MeanRev(Product):
    def __init__(self, symbol: str, limit: int, state: TradingState, z_th: float, window_size: int):
        super().__init__(symbol, limit, state)
        self.z_th = z_th
        self.window_size = window_size
        self.recent_mids = []
    
    def fair_val(self): # runtime polymorphism AGAIN
        raise NotImplementedError()
        ...
    
    def strategy(self):
        check_bid, check_ask, check_mid = self.best_bid(), self.best_ask(), self.mid_price()
        self.recent_mids.append(check_mid)
        if len(self.recent_mids) < self.window_size:
            return
        
        # z-score calculation
        z_score = (check_mid - statistics.mean(self.recent_mids)) / statistics.stdev(self.recent_mids)
        if z_score < -self.z_th:
            self.buy(check_bid, self.max_buy_orders())
        if z_score > self.z_th:
            self.sell(check_ask, self.max_sell_orders())

        # pop first elements from both
...
class Tomato(Product):
    ...
    def fair_val(self):
        ...
        # why worst? the outer two bid + ask are more balanced on this product
        # you're actually punished for adhering to the best bid / ask b/c those show more fluctuations
        if len(sell_orders) != 0 and len(buy_orders) != 0:
            return self.mid_price_using_worst()
        else:
            return math.nan
