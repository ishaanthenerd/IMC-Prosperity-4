...
def _normalize_trade_ids(trades: list[Trade]) -> None:
    """Own fills must use literal SUBMISSION on the submission side so logs / visualizers can classify."""
    for t in trades:
        if t.buyer is None:
            t.buyer = ""
        if t.seller is None:
...
def match_orders(
    ...
    for product in data.products:
        ...
        if len(new_trades) > 0:
            _normalize_trade_ids(new_trades)
    ...
    for product, trades in market_trades.items():
        ...
        _normalize_trade_ids(remaining_market_trades)
