...
class Arbitrage(Product):
    def __init__(self, product: str, limit: int, state: TradingState, lhs: List[Product], lhs_scalars: List[int], rhs: List[Product], rhs_scalars: List[int], z_th: float, window: int, fixed_mean1: float, fixed_mean2: float):
        super().__init__(product, limit, state)
        self.rolling_z_ltr = RollingZ(z_th = z_th, window = window, fixed_mean = fixed_mean1)
        self.rolling_z_rtl = RollingZ(z_th = z_th, window = window, fixed_mean = fixed_mean2)
        self.lhs = lhs
...
product_instances: Dict[Symbol, Product] = {}
